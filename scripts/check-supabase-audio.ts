...
async function collectFiles(bucket: Bucket, prefix: string, out: string[]): Promise<void> {
  ...
  for (let offset = 0; ; offset += PAGE) {
    ...
    for (const item of data) {
      ...
      if (item.id === null) subFolders.push(full)   // thư mục con → đệ quy sau
...
async function checkBucket(bucket: Bucket): Promise<Report> {
  ...
  const report: Report = { total: all.length, presentVps: 0, missingVps: [], inDb: 0, orphanStorage: 0 }
  ...
  console.log(`  • Đối chiếu DB          : có dòng ${report.inDb.toLocaleString('vi-VN')}  ·  KHÔNG có dòng (orphan storage) ${report.orphanStorage.toLocaleString('vi-VN')}`)

  return report
}

// ── Seed: tải các object Storage còn thiếu ở VPS về local ────────────────────
async function seedMissing(bucket: Bucket, missing: string[]): Promise<{ copied: number; errors: number }> {
  const targets = FORCE
    ? missing
  ...
  for (let i = 0; i < targets.length; i += DOWNLOAD_BATCH) {
    await Promise.all(targets.slice(i, i + DOWNLOAD_BATCH).map(async (obj) => {
      ...
      try {
        ...
        errors++
        if (samples.length < 5) samples.push(`${obj} → ${err instanceof Error ? err.message : String(err)}`)
      }
    }))
...
async function main(): Promise<void> {
  ...
  console.log('🔍 Kiểm tra audio Supabase Storage ↔ VPS local' + (SEED ? ' (sẽ SEED file thiếu)' : ' (chỉ báo cáo)'))
  console.log(`📁 Thư mục local: ${UPLOADS_ROOT}`)

  const buckets = resolveBuckets()
  let grandMissing = 0, grandCopied = 0, grandErrors = 0
  for (const bucket of buckets) {
    ...
    if (SEED && report.missingVps.length > 0) {
      ...
      grandCopied += r.copied; grandErrors += r.errors
    }
  }

  console.log('\n──────────────────────────────────────────────')
  if (SEED) {
    console.log(`📦 TỔNG: thiếu ${grandMissing.toLocaleString('vi-VN')} → tải về ${grandCopied.toLocaleString('vi-VN')}  ✗ lỗi ${grandErrors}`)
    if (grandErrors === 0 && grandCopied === grandMissing) console.log('✅ Đã kéo hết file Storage còn thiếu về VPS.')
  } else {
    console.log(`📦 TỔNG: VPS còn thiếu ${grandMissing.toLocaleString('vi-VN')} file so với Supabase Storage.`)
    if (grandMissing > 0) console.log('ℹ️  Chạy `npm run check:supabase -- --seed` để tải các file thiếu về VPS.')
