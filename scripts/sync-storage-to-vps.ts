...
function urlKeyFor(
  bucket: Bucket,
  parts: { lang?: string; voice: string; hash?: string; word?: string },
): string {
...
async function syncOne(
  ...
  if (DRY_RUN) {
    counters.copied++
    return
  } // dry-run: chỉ đếm "sẽ tải", không gọi mạng
...
async function syncBucket(bucket: Bucket): Promise<Counters> {
  ...
  for (let i = 0; i < keys.length; i += BATCH_SIZE) {
    await Promise.all(
      keys.slice(i, i + BATCH_SIZE).map((k) => syncOne(bucket, k, counters, samples)),
    )
  ...
  console.log(
    `  ⏭ Đã có local: ${counters.skip}  ${DRY_RUN ? 'sẽ tải' : '↓ Tải về'}: ${counters.copied}  ∅ Không có trên Supabase: ${counters.missingRemote}  ✗ Lỗi: ${counters.errors}`,
  )
  if (samples.length > 0) {
  ...
  if (bucket === 'tts-cache') {
    ...
    return rows.slice(0, LIMIT).map((r) => {
      ...
      return {
        ...
        update: (target) =>
          supabase.from('tts_cache').update({ audio_url: target }).eq('hash', r.hash),
      }
    })
  }
...
async function rewriteBucket(bucket: Bucket, base: string): Promise<RewriteCounters> {
  ...
  for (let i = 0; i < rows.length; i += UPDATE_BATCH) {
    await Promise.all(
      rows.slice(i, i + UPDATE_BATCH).map(async (row) => {
        const marker = `/uploads/${bucket}/${row.urlKey}`
        // Đã trỏ local (tương đối hoặc tuyệt đối đều chứa marker) → không đụng.
        if (row.audioUrl && row.audioUrl.includes(marker)) {
          counters.alreadyLocal++
          return
        }
        ...
        if (DRY_RUN) {
          counters.changed++
          if (samples.length < 5) samples.push(marker)
          return
        }
        ...
        if (error) {
          counters.errors++
          if (samples.length < 5) samples.push(`${marker} → ${error.message}`)
  ...
  console.log(
    `  ✏️ ${DRY_RUN ? 'sẽ đổi' : 'Đã đổi'}: ${counters.changed}  ⏭ đã local: ${counters.alreadyLocal}  ∅ chưa có file VPS: ${counters.noFile}  ✗ lỗi: ${counters.errors}`,
  )
  if (samples.length > 0) {
    console.log('     Ví dụ:')
    for (const s of samples) console.log(`       • ${s}`)
...
async function main(): Promise<void> {
  ...
  if (process.env.STORAGE_DRIVER !== 'local') {
    console.log(
      '⚠️  STORAGE_DRIVER hiện KHÔNG phải "local" — app chỉ phục vụ /uploads khi chạy chế độ local.',
    )
  }
  ...
  if (REWRITE_URLS) {
    const base = (process.env.REWRITE_BASE_URL || process.env.BASE_URL || '')
      .trim()
      .replace(/\/$/, '')
    console.log(
      `✏️  Đổi audio_url → local${base ? ` (base: ${base})` : ' (đường dẫn tương đối /uploads/...)'}` +
        (DRY_RUN ? ' — DRY-RUN' : ''),
    )
    const totals: RewriteCounters = { changed: 0, alreadyLocal: 0, noFile: 0, errors: 0 }
    for (const bucket of buckets) {
      const c = await rewriteBucket(bucket, base)
      totals.changed += c.changed
      totals.alreadyLocal += c.alreadyLocal
      totals.noFile += c.noFile
      totals.errors += c.errors
    }
    console.log('\n──────────────────────────────────────────────')
    console.log(
      `📦 TỔNG: ✏️ ${DRY_RUN ? 'sẽ đổi' : 'đã đổi'} ${totals.changed}  ⏭ đã local ${totals.alreadyLocal}  ∅ chưa có file VPS ${totals.noFile}  ✗ lỗi ${totals.errors}`,
    )
    if (DRY_RUN) console.log('ℹ️  DRY-RUN — chưa ghi DB. Bỏ --dry-run để đổi thật.')
    else if (totals.noFile > 0)
      console.log(
        `ℹ️  ${totals.noFile} dòng chưa có file ở VPS → chạy \`npm run sync:storage\` (tải file) trước rồi đổi URL lại.`,
      )
    else if (totals.errors === 0)
      console.log('✅ audio_url đã trỏ local cho mọi dòng có file ở VPS.')
    return
  }

  // ── Chế độ tải file (mặc định) ────────────────────────────────────────────
  console.log(
    '🔄 Tải audio Supabase Storage → VPS local' +
  ...
  for (const bucket of buckets) {
    ...
    totals.skip += c.skip
    totals.copied += c.copied
    totals.missingRemote += c.missingRemote
    totals.errors += c.errors
  }

  console.log('\n──────────────────────────────────────────────')
  console.log(
    `📦 TỔNG: ⏭ đã có ${totals.skip}  ${DRY_RUN ? 'sẽ tải' : '↓ tải về'} ${totals.copied}  ∅ thiếu trên Supabase ${totals.missingRemote}  ✗ lỗi ${totals.errors}`,
  )
  if (DRY_RUN) {
    console.log('ℹ️  Đây là DRY-RUN — chưa tải gì. Bỏ --dry-run để tải thật.')
  } else if (totals.missingRemote > 0) {
    console.log(
      `ℹ️  ${totals.missingRemote} file không có ở cả local lẫn Supabase → cần tạo lại bằng \`npm run seed:all -- --all\`.`,
    )
  } else if (totals.errors === 0) {
    console.log(
      '✅ Mọi file kỳ vọng đã có ở local trên VPS. Bước tiếp: npm run sync:storage -- --rewrite-urls',
    )
  }
}
