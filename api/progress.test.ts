...
// grant_weekly_ai_bonus cộng thưởng lượt tuần cho gói Free. Không cộng khi chỉ đồng bộ
...
describe('POST /api/progress — cộng thưởng lượt tuần khi phát hiện học thật', () => {
  it('learned dài ra so với bản cũ → gọi grant_weekly_ai_bonus', async () => {
    ...
    expect(findCall('grant_weekly_ai_bonus')).toBeTruthy()
  })

  it('cefrDialogues dài ra → cũng tính là học thật, gọi grant_weekly_ai_bonus', async () => {
    query.mockImplementation(async (sql: string) => {
      if (sql.includes('select learned, hard, cefr_grammar, cefr_dialogues'))
        return { rows: [EMPTY_PROGRESS_ROW] }
      return { rows: [] }
  ...
  it('gửi lại ĐÚNG dữ liệu cũ (không mảng nào dài ra) → KHÔNG cộng thưởng', async () => {
    ...
    const resp = await handler(makeRequest({ learned: ['apple'] }))
    expect(resp.status).toBe(200)
    expect(findCall('grant_weekly_ai_bonus')).toBeFalsy()
  })
  ...
  it('lỗi khi cộng thưởng (DB throw) → vẫn lưu tiến độ thành công (fail-open, không vỡ luồng chính)', async () => {
    query.mockImplementation(async (sql: string) => {
      if (sql.includes('select learned, hard, cefr_grammar, cefr_dialogues'))
        return { rows: [EMPTY_PROGRESS_ROW] }
      if (sql.includes('grant_weekly_ai_bonus')) throw new Error('db down')
      return { rows: [] }
    })
    const resp = await handler(makeRequest({ learned: ['apple'] }))
    expect(resp.status).toBe(200)
