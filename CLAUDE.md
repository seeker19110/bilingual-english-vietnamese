...
- [x] Chế độ Luyện nói song ngữ — TTS chính đã đổi sang Google Cloud TTS qua `/api/tts` (audio cache **mã hóa AES-256-GCM** trên Supabase Storage, bắt buộc đăng nhập mới lấy được khoá giải mã), Web Speech API chỉ còn là fallback khi lỗi mạng/server. **STT thật đã làm**: ghi âm ở trình duyệt (`MediaRecorder`, `src/lib/sttServer.ts`) → gửi base64 lên `/api/stt` → Whisper qua **Groq hoặc OpenAI** (`api/stt.ts` + `api/_lib/openaiStt.ts`, tự chọn theo key: có `GROQ_API_KEY` thì dùng Groq `whisper-large-v3-turbo`, không thì OpenAI `gpt-4o-mini-transcribe`); Web Speech API (`src/lib/stt.ts`) chỉ còn dự phòng khi máy không hỗ trợ ghi âm. Cần biến môi trường `GROQ_API_KEY` (hoặc `OPENAI_API_KEY`).
...
- [x] Lộ trình CHUẨN CEFR (tab "Lộ trình" trong `/learn`) — giáo trình A1→B2 ĐẦY ĐỦ: 21 unit / ~55 bài ngữ pháp cốt lõi (A1: to be→Wh-Q; A2: present/past/so sánh/mua sắm; B1: present perfect/tương lai/điều kiện/modal/mệnh đề quan hệ; B2: điều kiện 2-3/wish/bị động/causative/tường thuật/suy đoán/phrasal-idiom). Mỗi cấp có mục tiêu "can-do" (tiếng Việt); mỗi bài có cấu trúc + giải thích tiếng Việt + ví dụ bấm nghe. Liên kết TRỌN 34 vòng từ vựng (flashcard dùng chung `WordCard`, vẫn vào SRS + đếm lượt ngày). Dữ liệu: `src/data/cefr.ts`, UI: `src/components/RoadmapTab.tsx` (+ `WordCard.tsx` tách từ `Learn.tsx`). Đã thêm 3 vòng từ vựng mới cho B1-B2: travel, media, environment (`src/data/curriculum.ts`). Vẫn mở rộng được (thêm `grammar[]`/`units[]` là UI tự cập nhật). Còn làm: gắn nhãn CEFR cho phần từ vựng mở rộng, theo dõi % hoàn thành theo cấp.
- [x] (v2) Theo dõi tiến bộ, streak, chấm phát âm — đã có streak, WordOfTheDay, Flashcard, cache phát âm (`api/pronunciation.ts`); **chấm phát âm đã làm** (chạy ở trình duyệt bằng Web Speech + so chuỗi Levenshtein — `src/lib/pronounceScore.ts` + `src/components/PronunciationCheck.tsx`, gắn trong `/learn`)
- [x] Bảng tiến độ (`/progress`) — trang tổng hợp số liệu học: streak + biểu đồ cột 7 ngày gần nhất, mục tiêu từ mới hôm nay (x/20) + lượt còn lại (chat/nói/viết theo gói), số từ đã thuộc + cần ôn SRS + % lộ trình, **% hoàn thành từng cấp CEFR A1→B2** (tính theo số từ vựng đã thuộc của cấp, giống ngưỡng mở khóa 70% của RoadmapTab), tổng kết phiên chat/nói/bài viết. Logic gom số liệu: `src/lib/stats.ts`; UI: `src/pages/Dashboard.tsx`; lối vào từ trang chủ (nút "Tiến độ" cạnh "Lịch sử").
- [x] Tên miền canonical (SEO) đọc từ biến `VITE_SITE_URL` trong `src/App.tsx` (thay vì hard-code), mặc định domain production nếu không đặt — dễ dùng cho staging/domain khác. Xem `.env.example`.
- [x] **Hệ thống theme (màu giao diện) + audit UI** — màu nhấn thương hiệu (trước hard-code `emerald` ~250 chỗ) đã chuyển thành biến CSS `--a-*` (class `accent-*`, map trong `tailwind.config.js`) → đổi theme là đổi cả nền (zinc) lẫn nhấn (accent) chỉ bằng biến CSS. **4 theme, mặc định Xanh đêm**: 🌙 Xanh đêm (navy + emerald) · ☀️ Blue sky (sáng + sky) · 🌸 Pink (hồng phấn + pink) · 🎉 Rực rỡ (tím than + fuchsia, glow mạnh). Chọn theme bằng **menu có swatch** (`src/components/ThemeToggle.tsx`); định nghĩa theme ở `src/index.css` + `src/lib/theme.ts`; `applyTheme()` đồng bộ luôn `<meta theme-color>`. **Giữ nguyên màu ngữ nghĩa** (xanh lá="đúng", màu phân cấp A1-B2/loại từ trong `data/*.ts`, `pos.ts`, `pronounceScore.ts`). **Font đồng nhất**: bỏ cỡ <11px (gộp `text-[9px]`/`text-[10px]`→`text-[11px]`), quy `text-[16px]→base`, `text-[17px]→lg`; sửa icon `w-4.5` (class không hợp lệ) → `w-5`. **Lighthouse/a11y**: focus outline theo màu accent; `<meta theme-color>` đổi theo theme. **Zoom mobile: CHỦ ĐỘNG khóa** (`maximum-scale=1, user-scalable=no` — yêu cầu sản phẩm giữ layout app), chấp nhận đánh đổi 1 mục a11y "allow zoom" của Lighthouse; bù lại đã đặt sàn chữ ≥11px + input 16px để vẫn đọc tốt. (Kiểm chứng: build pass + chụp 4 theme.)

### Việc còn dang dở / cần quyết định
1. ~~STT chưa làm~~ → ĐÃ XONG (Whisper qua Groq/OpenAI tự chọn theo key, qua `/api/stt`). Còn lại: thêm `GROQ_API_KEY` (hoặc `OPENAI_API_KEY`) vào `.env` trên VPS + cân nhắc đếm lượt riêng cho STT (hiện dùng chung giới hạn `speaking`, đã có auth + rate limit chống lạm dụng).
