...
- [x] Deploy VPS (Express `server.ts` + PM2 + Nginx + Let's Encrypt) — ĐÃ deploy thật tại https://en-vi.donghanhcungban.com (PM2 process `english-tutor`, port 3001, dùng chung VPS 160.30.172.203 với app "xboss" ở port 3000 — không ảnh hưởng nhau). SSL Let's Encrypt tự renew. **[Cập nhật 2026-07-20]** Đã THỬ rồi ROLLBACK cluster mode: cluster mode (1 instance) + `wait_ready` từng được áp (PR #283/#284) để reload zero-downtime, nhưng worker crash im lặng khi chạy thật trên VPS (Node `cluster` module không tương thích với loader ESM `--import tsx`) → app down hẳn. Đã rollback về **fork mode** ổn định (PR #285) — `script: './node_modules/.bin/tsx'`, `interpreter: '/usr/bin/node'`, reload qua `scripts/pm2-reload.sh` chấp nhận vài giây downtime. **KHÔNG thử lại cluster mode** nếu chưa có cách nạp TypeScript khác tương thích Node cluster — xem chi tiết `PROGRESS.md` mục "Nợ kỹ thuật". (code + hướng dẫn: `docs/deploy-vps-ubuntu.md`)
