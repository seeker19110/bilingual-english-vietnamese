...
- [x] Chế độ Học theo lộ trình (`/learn`) — curriculum nền tảng theo vòng tròn chủ đề rồi nối tiếp bằng từ điển; tốc độ học tùy chọn 5/10/20 từ/ngày (mặc định 10 cho người dùng mới, người đã có tiến độ trước đó giữ 20 — chọn ở `/profile`, `getDailyGoal()`/`setDailyGoal()` trong `src/lib/curriculum.ts`); ôn ngẫu nhiên không lặp trong 1 vòng; học xong hiện câu thông dụng ráp từ các từ vừa học. Dữ liệu: `src/data/curriculum.ts`, logic: `src/lib/curriculum.ts`
- [x] Lộ trình CHUẨN CEFR — A1→B2 đầy đủ: 21 unit / ~55 bài ngữ pháp; mỗi cấp có mục tiêu "can-do", mỗi bài có cấu trúc + giải thích tiếng Việt + ví dụ bấm nghe; liên kết trọn 34 vòng từ vựng (flashcard `WordCard`, vào SRS + đếm lượt ngày). **Mỗi cấp 1 TRANG RIÊNG** `/learning-path/a1…b2` (`src/pages/CefrLevelPage.tsx`): thẻ "Học tiếp", unit theo trình tự ① Từ vựng → ② Ngữ pháp → ③ Hội thoại, mục hoàn thành 100% tự ẩn (xem lại được), bài ngữ pháp có nút "Đã học xong" (`src/lib/cefrProgress.ts` — đồng bộ Supabase qua cột `cefr_grammar`/`cefr_dialogues` của `learning_progress`, migration `0007`). Trang `/learning-path` = tổng quan 4 cấp (`RoadmapTab.tsx` + mốc từ vựng); màn chi tiết dùng chung ở `CefrLessonViews.tsx`. **4 tab học Hôm nay · Ôn SRS · Từ khó · Kiểm tra nằm TRONG trang từng cấp** (thanh tab đầu trang `CefrLevelPage.tsx`, nội dung `src/components/StudyTabs.tsx`) và lọc dữ liệu THEO TỪ VỰNG CỦA CẤP (`getLevelWords()`; cấp cuối B2 học tiếp phần ngoài CEFR — `getBeyondCefrWords()`); giới hạn ngày (tốc độ học 5/10/20 từ/lượt tùy chọn, trần = 5×tốc độ) vẫn tính chung toàn app. Dữ liệu: `src/data/cefr.ts`. Còn: TỪ VỰNG MỞ RỘNG chưa có nhãn CEFR trên từng từ (hiện chỉ liên kết qua `vocabCircleIds` ở cấp unit) — hạ tầng gắn nhãn bằng AI đã có (`scripts/tag-cefr-levels.ts`, field `DictEntry.level`) nhưng CHƯA chạy thật (cần key AI + xác nhận người dùng, xem PROGRESS.md).
