...
1. ~~STT chưa làm~~ → ĐÃ XONG (OpenAI `gpt-4o-mini-transcribe` qua `/api/stt`). Còn lại: thêm `OPENAI_API_KEY` vào `.env` trên VPS + cân nhắc đếm lượt riêng cho STT (hiện dùng chung giới hạn `speaking`, đã có auth + rate limit chống lạm dụng).
