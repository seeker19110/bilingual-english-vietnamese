...
- [~] Lộ trình CHUẨN CEFR (tab "Lộ trình" trong `/learn`) — khung A1→B2: mỗi cấp có mục tiêu "can-do" (tiếng Việt), các unit gồm bài NGỮ PHÁP (cấu trúc + giải thích tiếng Việt + ví dụ bấm nghe) và liên kết tới các vòng từ vựng sẵn có (flashcard dùng chung `WordCard`, vẫn vào SRS + đếm lượt ngày). Dữ liệu: `src/data/cefr.ts`, UI: `src/components/RoadmapTab.tsx` (+ `WordCard.tsx` tách từ `Learn.tsx`). **Đây là KHUNG — ngữ pháp đã có nội dung thật nhưng còn mở rộng được** (thêm bài vào `grammar[]`/`units[]` là UI tự cập nhật). Còn làm: bổ sung nội dung từng cấp, gắn nhãn CEFR cho phần từ vựng mở rộng, theo dõi % hoàn thành theo cấp.
