...
- **PR 1 — Backend Chat**: migration `chat.*` (rooms/room_members/messages/moderation_events),
  `packages/core-chat/` (moderator VI+EN wordlist, chatService — **gọi `areFriends()` trước khi tạo
  DM room**, wsHandler qua cookie auth, redisChat có fallback single-process), zod contracts, mount
  WS vào `http.Server` của `server.ts`.
- **PR 2 — Frontend Chat UI**: `ChatPage.tsx` + components, `useChat.ts` hook, route `/chat`, E2E.
- ⚠️ Việc tay sau này: cài Redis + `REDIS_URL` trên VPS để fan-out multi-instance hoạt động thật
  (trước đó vẫn chạy được nhờ fallback single-process, chỉ chưa scale nhiều tiến trình).
