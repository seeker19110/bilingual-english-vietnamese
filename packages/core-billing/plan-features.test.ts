...
vi.mock('../../api/_lib/planFeatures', () => ({ getPlanFeatureMatrix: vi.fn() }))
...
async function importHandler() {
  vi.resetModules()
  const mod = await import('./plan-features')
  return mod.default
}

describe('/api/plan-features', () => {
  it('không cần đăng nhập, trả ma trận tính năng + ETag', async () => {
    const { getPlanFeatureMatrix } = await import('../../api/_lib/planFeatures')
