...
describe('generateAudioFromGoogle — xoay vòng key', () => {
  it('không có key nào → throw lỗi rõ ràng', async () => {
    ...
    const { generateAudioFromGoogle } = await import('./googleTts.js')
...
describe('probeApiKeys + setActiveKeyPool', () => {
  it('probe từng key song song, trả về key nào 200 và key nào lỗi', async () => {
    ...
    const { probeApiKeys } = await import('./googleTts.js')
  ...
  it('setActiveKeyPool thu hẹp bể — chỉ xoay vòng qua key được truyền vào (sau probe)', async () => {
    ...
    const { generateAudioFromGoogle, setActiveKeyPool } = await import('./googleTts.js')
...
describe('hasGoogleTtsKey', () => {
  it('false khi chưa cấu hình key nào', async () => {
    delete process.env.GOOGLE_TTS_API_KEY
    delete process.env.GOOGLE_TTS_API_KEYS
    const { hasGoogleTtsKey } = await import('./googleTts.js')
...
describe('generateStudioAudioFromGoogle — giọng cao cấp, CHỈ tiếng Anh', () => {
  it('gửi đúng tên giọng en-US-Studio-O/Q (không có "-Chirp3-HD-" như giọng thường)', async () => {
    ...
    const { generateStudioAudioFromGoogle } = await import('./googleTts.js')
...
describe('isValidStudioVoice', () => {
  it('nhận đúng Studio-O/Studio-Q, từ chối giọng Chirp3-HD thường và chuỗi bất kỳ', async () => {
    const { isValidStudioVoice } = await import('./googleTts.js')
...
describe('canonicalizeVoiceId', () => {
  it('đưa mọi cách viết hoa/thường về đúng tên giọng chuẩn', async () => {
    const { canonicalizeVoiceId } = await import('./googleTts.js')
