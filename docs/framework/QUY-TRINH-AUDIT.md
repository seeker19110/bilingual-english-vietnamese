...
| # | Nhóm (theo WSTG) | Kiểm tra cụ thể | Cách kiểm trong repo này | Đã biết đạt (2026-08-04) |
|---|---|---|---|---|
| 1 | Injection — SQLi | Mọi câu Postgres dùng tham số hoá (`$1, $2…`), không nối chuỗi trực tiếp | `grep -rn "pool.query\|client.query" api/ packages/core-db --include=*.ts \| grep -v "\.test\."` rồi soát các query có nối chuỗi (`+`, template literal chèn biến chưa qua `$n`) | cần soát lại mỗi lần thêm handler mới |
| 2 | Injection — Command/Path | Không có `child_process.exec`/`fs` ghép trực tiếp input người dùng vào đường dẫn | `grep -rn "exec(\|execSync\|readFile.*req\.\|path.join.*req\." api/ server.ts` | STT/TTS ghi file cache theo hash, không theo tên người dùng gửi lên — cần xác nhận lại nếu đổi `fileStorage.ts` |
...
| 4 | CSRF | Endpoint đổi trạng thái (POST/PUT/DELETE) yêu cầu Bearer token riêng (không chỉ dựa cookie tự động gửi) → giảm rủi ro CSRF theo thiết kế token-based hiện tại | Xác nhận `validateAuth()` đọc header `Authorization`, không phải cookie session | Đúng — auth Bearer token, không cookie, nên CSRF cổ điển không áp dụng; **ngoại lệ cần soát riêng: webhook thanh toán** (`payment-webhook`) — xác minh chữ ký SePay, không dựa "đăng nhập" |
| 5 | Auth — brute force / enumeration | Endpoint login/đăng ký không lộ "email tồn tại hay không" qua message khác nhau; có giới hạn số lần thử | Đọc `api/auth.ts` + `api/_lib/authService.ts`, đối chiếu message lỗi + rate limit áp dụng cho `/api/auth*` | cần audit đọc lại 2 file này khi chạy tầng này (chưa có bằng chứng gần đây trong PROGRESS) |
| 6 | Session/Token | Token có hạn dùng (expiry), không log token ra console/Sentry, đăng xuất thu hồi được | `grep -rn "console.log.*token\|Bearer " api/ --include=*.ts \| grep -v test` | cần soát; Sentry cấu hình `beforeSend` có lọc header Authorization không → đọc `packages/core-ai` hoặc file init Sentry |
| 7 | Authorization / IDOR | Mọi handler đọc/sửa dữ liệu theo `id` từ URL/body đều đối chiếu `user_id` lấy từ token, không tin `user_id` client gửi lên | Đã có ở Tầng 2 gốc ("Kiểm quyền mỗi handler") — bổ sung: rà riêng các handler có tham số `:id`/`userId` trong query string (VD `history.ts`, `progress.ts`, `profile.ts`) xem có so khớp `req.user.id` hay dùng thẳng param | cần liệt kê danh sách handler có param id khi chạy tầng này |
| 8 | File upload (STT audio) | Giới hạn kích thước + loại file audio nhận vào `/api/stt`; không cho ghi ra ngoài thư mục cache dự kiến | Đọc `api/stt.ts` + `api/_lib/openaiStt.ts`: có giới hạn `bodyParser`/base64 size không, có validate mime/độ dài trước khi gửi Whisper | cần đọc lại — chưa có ghi nhận giới hạn kích thước trong PROGRESS |
| 9 | Security headers | CSP, `X-Content-Type-Options`, `Referrer-Policy` đã có (`server.ts`); còn thiếu `X-Frame-Options`/`frame-ancestors`, `Strict-Transport-Security`, `Permissions-Policy` | `grep -n "X-Frame\|Strict-Transport\|Permissions-Policy\|frame-ancestors" server.ts` | **CSP + nosniff + Referrer-Policy: có** (server.ts dòng ~144-174). `X-Frame-Options`/`frame-ancestors`/HSTS/`Permissions-Policy`: cần xác nhận — HSTS có thể do Nginx/Let's Encrypt set sẵn ở tầng ngoài, không phải Express; audit lượt tới cần đọc cấu hình Nginx trên VPS để chốt, không tự thêm trùng ở Express nếu Nginx đã set |
| 10 | CORS | Nếu có bật CORS cho origin khác domain chính, danh sách origin phải whitelist rõ, không `*` khi có credentials | `grep -rn "Access-Control-Allow" server.ts api/` | dự án là same-origin SPA (không gọi cross-origin từ domain khác) — xác nhận không có middleware CORS mở rộng nào khi chạy tầng này |
| 11 | Rate limiting / anti-automation | Endpoint tốn tài nguyên (AI, STT, TTS, auth) có giới hạn lượt/IP hoặc /user, tách biệt đếm lượt nghiệp vụ (Free/Pro) và rate-limit chống spam kỹ thuật | Đọc `api/_lib/http.ts` (nơi có `rate` theo grep ở trên) — xác nhận có áp cho `/api/auth`, `/api/agent`, `/api/stt`, `/api/tts`, không chỉ đếm lượt nghiệp vụ | cần đọc `api/_lib/http.ts` khi chạy tầng này để chốt phạm vi áp dụng |
| 12 | Business logic (đếm lượt/thanh toán) | Không thể gọi API AI vượt giới hạn bằng cách gọi song song (race condition đếm lượt), webhook thanh toán xác minh chữ ký + idempotent (không cộng tiền 2 lần nếu SePay gửi lại) | Đã có phần trong Tầng 5b ("Async race / idempotency… `0004_refund_usage`") — bổ sung riêng cho `payment-webhook`: xác nhận có kiểm tra "đã xử lý giao dịch này chưa" trước khi cộng plan | cần đối chiếu `api/payment-webhook.ts` (hoặc tên tương đương) khi chạy tầng này |
| 13 | Error handling / thông tin lộ | Response lỗi cho client không kèm stack trace / chi tiết nội bộ (tên bảng, path server) ở production | `grep -rn "err.stack\|error.stack" api/ server.ts \| grep -v test` rồi xác nhận không trả thẳng ra response | cần soát khi chạy tầng này |
| 14 | Sensitive data ở client | Không có API key/secret nào lộ trong bundle client (`apps/english/src`) — chỉ biến `VITE_*` public (site URL, Google client ID publishable) | `grep -rn "process.env\." apps/english/src \| grep -v VITE_` (phải rỗng — code client không được đọc biến server) | cần chạy grep này mỗi audit — chưa có trong Tầng 2 gốc, bổ sung chính thức từ nay |

**Ghi chú phạm vi:** danh mục trên ưu tiên các lớp lỗ hổng OWASP Top 10 còn *chưa* có dòng kiểm tra rõ
