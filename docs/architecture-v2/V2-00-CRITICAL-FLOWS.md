...
| - | ---- | ------------------ | -------------------- | ----- | ----- |
| 1 | Payment webhook (1.6) — ĐÃ ĐỌC CODE XÁC NHẬN: `payment-webhook.ts` KHÔNG dùng transaction Postgres (`BEGIN`/`COMMIT`). `UPDATE payments SET status='paid' ... WHERE status='pending'` là atomic 1 câu lệnh (chống trùng đúng), nhưng `grantPlanDays()` (ghi `entitlements`) chạy SAU đó, TÁCH RỜI. Nếu `grantPlanDays()` throw (lỗi DB tạm thời, mất kết nối...) → `payments.status` đã = `'paid'` nhưng user CHƯA được cấp gói, và vì `status='paid'` nên lần webhook retry sau (SePay retry tới 7 lần) sẽ bị nhánh `if (payment.status === 'paid') return ok(headers)` chặn — không tự phục hồi được, phải xử lý tay | Bất kỳ lỗi DB thoáng qua đúng lúc `grantPlanDays()` chạy | Bọc `UPDATE payments` + `grantPlanDays()` trong 1 transaction thật (`core-db/transaction.ts` đã có sẵn từ Phase 01, dùng ngay được) — đây là bug production thật, không phải suy đoán; sửa RIÊNG 1 PR nhỏ, KHÔNG gộp vào Wave A (goal hiện tại cấm sửa `packages/core-billing`) | seeker19110 (quyết định có ưu tiên PR fix ngay hay để backlog) | OPEN — xác nhận có bug, chưa fix |
