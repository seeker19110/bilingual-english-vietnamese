...
if nhiet_do >= 25 and nhiet_do < 35:
    ...
    parsons: {
      prompt:
        'Xếp đúng thứ tự bậc thang xếp loại: đọc điểm → xét Giỏi → xét Khá → còn lại là Yếu.',
      lines: [
...
print(f"{b.ten}: {b.trung_binh()}") # 5 / 1 = 5.0`,
    ...
    parsons: {
      prompt: 'Xếp đúng thứ tự: định nghĩa lớp với __init__ và phương thức, rồi tạo đối tượng và gọi.',
      lines: [
        'class HocVien:',
