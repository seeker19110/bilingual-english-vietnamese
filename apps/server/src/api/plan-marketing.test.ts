...
vi.mock('@dhcb/core-billing/planMarketing', () => ({ getPlanMarketing: vi.fn() }))
...
async function importHandler() {
  vi.resetModules()
  const mod = await import('./plan-marketing.js')
  return mod.default
}

describe('/api/plan-marketing', () => {
  it('không cần đăng nhập, trả nội dung mô tả gói + ETag', async () => {
    const { getPlanMarketing } = await import('@dhcb/core-billing/planMarketing')
