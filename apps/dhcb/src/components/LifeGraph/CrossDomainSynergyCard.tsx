        ...
        {data.synergies.map((item) => (
          ...
          >
            ...
            {item.suggestedAction && (
              <div className="flex items-center gap-1.5 text-[11px] font-medium text-sky-300 bg-zinc-950/60 p-2 rounded-lg border border-zinc-800/60">
      ...
      {data.conflicts.length > 0 && (
        <div className="space-y-2.5 pt-2 border-t border-zinc-800/80">
          ...
          {data.conflicts.map((con) => (
            <div
              key={con.id}
              className="p-3 rounded-xl bg-amber-500/10 border border-amber-500/20 space-y-1.5 text-xs text-amber-200"
            >
