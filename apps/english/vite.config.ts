...
// File này nằm ở apps/english/ (dời từ gốc repo, PR-S2) — mọi đường dẫn tính từ đây.
...
export default defineConfig(({ mode }) => {
  ...
  return {
    ...
    // qua `vite --config apps/english/vite.config.ts` (npm script gốc giữ nguyên tên lệnh).
    root: appDir,
    resolve: {
      alias: [
