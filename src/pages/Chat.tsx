...
import { Send, Plus, ChevronDown, Sparkles, Award, Target } from 'lucide-react'
...
function SetupScreen({
  ...
  defaultLevel?: Level
  // Từ vừa học ở lộ trình (mục B — nối lộ trình ↔ Chat), truyền qua điều hướng.
  targetWords?: string[]
}) {
  // Có từ mục tiêu → mặc định "Tự do" để AI rảnh tay dẫn hội thoại quanh các từ này.
  const [situation, setSituation] = useState(targetWords?.length ? 'free' : 'job_interview')
      ...
      <div className="w-full max-w-sm space-y-4 animate-fade-up delay-150">
        ...
        {targetWords && targetWords.length > 0 && (
          <div className="bg-teal-500/10 border border-teal-500/30 rounded-xl px-4 py-3">
            <div className="flex items-center gap-1.5 mb-1.5 text-teal-300">
            ...
            <p className="text-xs text-teal-200/80">{targetWords.join(', ')}</p>
          </div>
        )}
...
export default function Chat() {
  ...
  async function startSession(situation: string, level: Level) {
    ...
    setLoading(true)
    setError('')
    const sys = chatSystemPrompt(situationLabel(situation, dir), level, dir, targetWords)
    try {
