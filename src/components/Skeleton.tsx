...
export function Skeleton({ className = '' }: { className?: string }) {
  return (
    <div
      ...
      style={{ backgroundImage: 'linear-gradient(90deg, rgba(63,63,70,0) 0%, rgba(82,82,91,0.5) 50%, rgba(63,63,70,0) 100%)' }}
    />
  )
}
  ...
  return (
    <div className="max-w-3xl mx-auto w-full px-4 py-6 space-y-3">
      {Array.from({ length: rows }).map((_, i) => (
        <div key={i} className="flex items-center gap-4 rounded-2xl border border-zinc-800/80 bg-zinc-900/60 p-4">
