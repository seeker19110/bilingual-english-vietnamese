...
export default function QuickActions() {
  ...
  const notifLabel = !supported
    ? (isA ? 'Không hỗ trợ' : 'Unsupported')
    : pushLoading
      ? '…'
      : pushOn
        ? fmtHour(remindHour)
        : (isA ? 'Nhắc học' : 'Remind me')
  ...
  return (
    <div className="mt-8 pt-5 border-t border-zinc-800/60">
      <div className="grid grid-cols-3 gap-2 max-w-md mx-auto">
        ...
        <button onClick={onNotifClick} disabled={!supported || pushLoading}
          aria-label={pushOn ? (isA ? 'Tắt nhắc học' : 'Turn off reminders') : (isA ? 'Bật nhắc học mỗi ngày' : 'Turn on daily reminders')}
      ...
      <p className="text-[11px] text-zinc-500 text-center mt-2.5 max-w-md mx-auto">
        🔔 {pushOn
          ? (isA ? `Sẽ nhắc bạn vào học lúc ${fmtHour(remindHour)} mỗi ngày để giữ chuỗi 🔥` : `We'll nudge you to study at ${fmtHour(remindHour)} daily to keep your 🔥 streak`)
          : (isA ? 'Nhắc bạn vào học mỗi ngày để giữ chuỗi 🔥 ngày liên tiếp' : 'A daily nudge to study and keep your 🔥 streak')}
      </p>
      ...
      {showTime && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/60 backdrop-blur-sm px-4"
          ...
          <div className="w-full max-w-xs bg-zinc-900 border border-zinc-700/60 rounded-2xl p-5 animate-fade-in"
            ...
            <div className="flex items-center justify-between mb-1">
              ...
              <button onClick={() => setShowTime(false)} aria-label={isA ? 'Đóng' : 'Close'}
                className="text-zinc-400 hover:text-white p-1 -mr-1">
                <X className="w-4 h-4" />
              </button>
            </div>
            <p className="text-[11px] text-zinc-400 mb-3">
              {isA ? 'Mỗi ngày tới giờ này, nếu bạn chưa học, app sẽ nhắc nhẹ một cái.' : "Each day at this time, if you haven't studied, we'll send a gentle nudge."}
            </p>
            ...
            <button onClick={confirmTime} disabled={pushLoading}
              ...
              {pushLoading ? '…' : (isA ? `Nhắc tôi lúc ${fmtHour(remindHour)}` : `Remind me at ${fmtHour(remindHour)}`)}
            </button>
          </div>
        </div>
      )}
      ...
      {showShare && (
        <ShareProgress userId={userId} isA={isA} onClose={() => setShare(false)} />
      )}
    </div>
  )
}
