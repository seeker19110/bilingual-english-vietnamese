...
- Đăng nhập/đăng ký qua Supabase Auth (`src/lib/auth.ts`).
...
- **Bảo mật:** không tin client; xác thực/đếm lượt/gọi AI đều ở server (`api/`, `server.ts`,
  `api/_lib/security.ts`, `api/_lib/usage.ts`); RLS Supabase trên mọi bảng người dùng (kể cả
  chặn theo CỘT cho `profiles.plan`/`daily_usage.*_count` — chỉ server ghi được); secret qua env.
...
- **Backend & dữ liệu:** Express (`server.ts`) + Supabase (Auth, Postgres RLS, Storage). Handler
  API kiểu serverless trong `api/`, gắn vào Express để chạy trên VPS.
- **AI:** chat/chấm viết qua `/api/claude` (`api/ai.ts`, ép model/token ở server) · STT Whisper
  ...
  TTS Google Cloud (`/api/tts`), cache **mã hoá AES-256-GCM** trên Supabase Storage, Web Speech
  API chỉ là fallback.
...
- **Luồng:** client (React SPA) → `server.ts` (Express, gắn handler `api/*.ts`) → provider AI
  (Claude/Groq/OpenAI/Google TTS) + Supabase. Mọi logic nhạy cảm chạy ở server.
...
nhánh xử lý; đổi schema Supabase phải có migration idempotent chạy lại an toàn + cách rollback.
